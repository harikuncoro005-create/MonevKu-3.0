            ...
            <div class="my-2">
                ...
                <div>
                    <select id="otorisasi" name="otorisasi[]" class="form-control" multiple="multiple">
                        @php
                            $selectedValues = is_array($result->admin_otorisasi) 
                                            ? $result->admin_otorisasi 
                                            : json_decode($result->admin_otorisasi, true) ?? [];
                        @endphp
                        @foreach ($instansi as $item)
                            <option value="{{ $item->instansi_kode }}" {{ in_array($item->instansi_kode, $selectedValues) ? 'selected' : '' }}>{{ $item->instansi_nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="my-2">
                ...
                <div>
                    <select id="role" name="role" class="form-control">
                        <option value="1" {{ $result->admin_role == '1' ? 'selected' : '' }}>User</option>
                        <option value="2" {{ $result->admin_role == '2' ? 'selected' : '' }}>Admin</option>
                        <option value="7" {{ $result->admin_role == '7' ? 'selected' : '' }}>Administrator</option>>
                    </select>
                </div>
            </div>
            <hr>
