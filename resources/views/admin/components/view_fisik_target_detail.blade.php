    ...
    <table class="table table-bordered">
        ...
        <tbody class="text-small text-gray-500" style="line-height: 1;">
            ...
            <tr class="text-center font-weight-bold alert-warning">
                ...
                <td colspan="13" style="vertical-align: middle;"></td>
            </tr>
        ...
        <tfoot class="text-small text-gray-500" style="line-height: 1;">
            ...
            <tr class="text-center">
                ...
                <td class="{{ collect($fisik->collapse())->sum('fisik_acuan') <=> 100 ? 'alert-danger' : 'alert-success' }}" style="vertical-align: middle;">{{ str_replace(".", ",", sprintf('%.2f', collect($fisik->collapse())->sum('fisik_acuan'))) }}</td>
                @foreach ($bulan as $index => $item)
                    <td style="vertical-align: middle; width: 5rem;">{{ str_replace(".", ",", sprintf('%.2f', collect($fisik->collapse())->sum('fisik_'.$index))) }}</td>
                @endforeach
            </tr>
            <tr class="text-center">
                ...
                <td></td>
                @foreach ($bulan as $index => $item)
                    <td class="font-weight-bold" style="vertical-align: middle; width: 5rem;">{{ str_replace(".", ",", sprintf('%.2f', $fisik_kumulatif[$index])) }}</td>
                @endforeach
            </tr>
            <tr class="text-center">
