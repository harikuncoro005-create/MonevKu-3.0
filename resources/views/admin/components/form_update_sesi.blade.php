...
<form id="form-add" autocomplete="off">
    <div class="modal-body">
        ...
        <hr>
        <input type="hidden" class="form-control" name="id" value="{{ $result->sesi_id }}">
        <input type="hidden" class="form-control" name="tahun" value="{{ $result->sesi_tahun }}">
        <div style="line-height: 1">
            <div class="my-2">
                <div class="text-gray-400 text-small">Status <small class="text-danger">*</small></div>
                <div>
                    <select name="status" id="status" class="form-control">
                        <option value="0" {{ $result->sesi_status == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                        <option value="1" {{ $result->sesi_status == 1 ? 'selected' : '' }}>Aktif</option>
                    </select>
                </div>
            </div>
            <div class="my-2">
                ...
                <div>
                    <input type="text" id="nama" name="nama" class="form-control" value="{{ $result->sesi_nama }}">
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Tanggal <small class="text-danger">*</small></div>
                <div>
                    <input type="text" id="tanggal" name="tanggal" class="form-control bg-white datepicker" value="{{ \Carbon\Carbon::parse($result->sesi_tanggal)->format('d/m/Y') }}" readonly>
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Keterangan</div>
                <div>
                    <textarea type="text" id="keterangan" name="keterangan" class="form-control">{{ $result->sesi_keterangan }}</textarea>
                </div>
            </div>
        </div>
        <br>
