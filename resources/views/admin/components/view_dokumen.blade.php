@if ($filename != '' && file_exists(public_path('/assets/img/'.$path.$filename)))
    @php
        $infoPath = pathinfo(public_path('/assets/img/'.$path.$filename));
        $extension = $infoPath['extension'];
    @endphp
    
    @if ($extension == 'pdf')
        <iframe src="/assets/img/{{ $path.$filename }}" width="100%" height="800"></iframe>
    @elseif ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png')
        <img class="rounded-full border-4 border-cyan-500" src="/assets/img/{{ $path.$filename }}">
    @else
