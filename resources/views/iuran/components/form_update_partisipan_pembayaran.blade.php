...
<form id="form-add" autocomplete="off">
    <div class="modal-body">
        ...
        <div style="line-height: 1">
            <input type="hidden" name="id" class="form-control" value="{{ $result->pembayaran_id }}">
            <input type="hidden" name="partisipan_id" class="form-control" value="{{ $result->pembayaran_partisipan_id }}">
            <div class="my-2">
                ...
                <div>
                    <select id="pembayaran" name="pembayaran" class="form-control">
                        <option value="0" {{ $result->pembayaran_tipe == '0' ? 'selected' : '' }}>Cash</option>
                        <option value="1" {{ $result->pembayaran_tipe == '1' ? 'selected' : '' }}>Transfer</option>
                    </select>
                </div>
            </div>
            <div class="my-2">
                ...
                <div>
                    <input type="text" id="jumlah" name="jumlah" class="form-control number" value="{{ $result->pembayaran_jumlah }}">
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Keterangan </div>
                <div>
                    <textarea id="keterangan" name="keterangan" class="form-control">{{ $result->pembayaran_keterangan }}</textarea>
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Tanggal <small class="text-danger">*</small></div>
                <div>
                    <input type="text" id="tanggal" name="tanggal" class="form-control bg-white datepicker" readonly value="{{ \Carbon\Carbon::parse($result->pembayaran_tanggal)->format('d/m/Y') }}">
                </div>
            </div>
            <div class="my-2" id="form-upload">
                ...
                <div class="text-gray-400 text-small">Bukti Pembayaran</div>
                <div>
                    <div class="d-flex flex-row justify-content-between border border-success rounded px-3 py-2 text-small mb-2" style="line-height: 1">
                        <div class="text-success">{{ $result->pembayaran_dokumen }}</div>
                        <div>
                            <a href="" data-id="{{ $result->pembayaran_id }}" do="delete-attachment" class="text-gray-400 text-decoration-none btn-delete-attachment">
                    ...
                    <script type="text/javascript">
                        $('.btn-delete-attachment').off('click').on('click', function(e) {
                            ...
                            $.ajax({
                                ...
                                url: "/form-partisipan-pembayaran",
                                method:"POST",
                                data: {
                                    parameter: {
                                        to: $(this).attr('do'),
