...
<div class="bg-white rounded px-4 py-3 my-3 shadow-sm">
    <div class="d-flex flex-column flex-sm-row justify-content-between" style="row-gap:0.5rem">
        <div class="d-flex flex-column flex-sm-row align-items-center" style="column-gap:0.5rem; row-gap: 0.5rem;">
            <div class="w-100">
                <a href="/iuran" class="btn rounded px-4 text-blue-500 w-100 shadow-sm text-nowrap hover-blue-500"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
            </div>
            <div class="w-100">
                <a href="#" data-id="{{ $data->iuran_id }}" do="create" title="Tambah {{ $titlePage }}" class="btn px-4 btn-blue-500 w-100 shadow-sm text-nowrap btn-add"><i class="fa-solid fa-plus"></i> Tambah</a>
            </div>
            <div class="w-100">
                <a href="/iuran/rekap?id={{ $data->iuran_id }}" class="btn rounded px-4 text-gray-400 shadow-sm w-100 hover-blue-500 text-nowrap"><i class="fa-solid fa-chart-line"></i> Rekap</a>
            </div>
        </div>
...
<div class="bg-white rounded p-4 my-3 shadow-sm">
    <div class="text-gray-400 text-small d-flex d-inline justify-content-center px-3 py-1 border rounded" style="column-gap: 0.5rem">
        Total : <div id="total">{{ $total }}</div>
    </div>
    <hr>
    <div class="mb-2 d-flex justify-content-between align-items-center flex-row" style="column-gap: 0.5rem;">
        ...
        <div style="min-width:8rem">
            <select class="form-control search" param="kategori">
                <option value="" <?= !Request::get('kategori') ? 'selected' : '' ?>>Semua</option>
                <option value="1" <?= Request::get('kategori') && Request::get('kategori') == '1' ? 'selected' : '' ?>>Pemilik</option>
                <option value="2" <?= Request::get('kategori') && Request::get('kategori') == '2' ? 'selected' : '' ?>>Penyewa</option>
                <option value="3" <?= Request::get('kategori') && Request::get('kategori') == '3' ? 'selected' : '' ?>>Umum</option>
            </select>
        </div>
    </div>
        ...
        <table class="table table-hover table-bordered">
                <thead style="font-weight: 500; font-size: 0.9rem; line-height: 1rem; background-color: #f9fafb">
                    <tr class="text-center">
                        ...
                        <td style="vertical-align: middle; width: 15rem;">NAMA</td>
                        <td style="vertical-align: middle;">NOMOR RUMAH / ALAMAT</td>
                        <td style="vertical-align: middle; width: 5rem;">KATEGORI</td>
