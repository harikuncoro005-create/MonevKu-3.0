<table>
    <thead>
        <tr>
            <th colspan="44">LAPORAN RENCANA AKSI APBD TAHUN {{ session('session_kode')->sesi_tahun }}</th>
        </tr>
        <tr>
            <th colspan="44">{{ $instansi->instansi_nama }}</th>
        </tr>
        <tr>
            <td colspan="44"></td>
        </tr>
        ...
        <tr class="text-center small">
            @foreach ($bulan as $index => $value)
                <td style="vertical-align: middle; min-width: 3rem;">kin</td>
                <td style="vertical-align: middle; min-width: 3rem;">fis</td>
                <td style="vertical-align: middle; min-width: 6rem;">keu</td>
            @endforeach
            <td style="vertical-align: middle; min-width: 3rem;">kin</td>
            <td style="vertical-align: middle; min-width: 3rem;">fis</td>
            <td style="vertical-align: middle; min-width: 6rem;">keu</td>
    ...
    <tbody class="text-small text-gray-500" style="line-height: 1;">
        ...
        <?php } else {
            foreach ($akun['akun_2'] as $val_2) { ?>
                <tr>
                    <td style="background-color: #D1D5DB;mso-number-format:'\@'; "><strong><?= strval($val_2['akun_kode']) ?></strong></td>
                    <td style="background-color: #D1D5DB;"><strong><?= $val_2['akun_nama'] ?></strong></td>
                    <td colspan="42" style="background-color: #D1D5DB;"></td>
                </tr>
                <?php 
                    ...
                    <?php if(count($akun_5) > 0) {
                        ...
                        <?php if(count($akun_6) > 0) {
                            ...
                            <tr style="line-height: 1;" class="small">
                                ...
                                <td><em><?= $val_6['akun_nama'] ?></em></td>

                                <td>{{ $keluaran->has($val_6['akun_kode']) ? $keluaran->get($val_6['akun_kode'])->keluaran_nama : '' }}</td>
                                <td class="text-center">{{ $keluaran->has($val_6['akun_kode']) ? $keluaran->get($val_6['akun_kode'])->keluaran_target : '' }}</td>
                                <td class="text-center">{{ $keluaran->has($val_6['akun_kode']) ? $keluaran->get($val_6['akun_kode'])->keluaran_satuan : '' }}</td>
                                ...
                                @foreach ($bulan as $index => $value)
                                    ...
                                    <td class="text-right" style="vertical-align:">{{$keuangan->has($val_6['akun_kode']) ? $keuangan->get($val_6['akun_kode'])->{'keuangan_'.$index} : 0 }}</td>
                                @endforeach
                                ...
                                <td class="text-right" style="vertical-align: middle;">
                                    {{ $keuangan->has($val_6['akun_kode']) ? $keuangan->get($val_6['akun_kode'])->keuangan_pagu : 0 }}
                                </td>
