<form id="form-add" autocomplete="off">
    <div class="modal-body">
        ...
        <div style="line-height: 1">
            <input type="hidden" class="form-control" name="keluaran_id" value="{{ $realisasi_keluaran->keluaran_id }}">
            ...
            <div class="my-2">
                ...
                <div>
                    <textarea id="keluaran" name="keluaran" class="form-control" readonly>{{ $realisasi_keluaran->keluaran_nama }}</textarea>
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Satuan <small class="text-danger">*</small></div>
                <div>
                    <input type="text" id="satuan" name="satuan" class="form-control" value="{{ $realisasi_keluaran->keluaran_satuan }}"  readonly>
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Target <small class="text-danger">*</small></div>
                <div>
                    <input type="text" id="target" name="target" class="form-control" value="{{ $realisasi_keluaran->keluaran_target }}"  readonly>
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Realisasi Bulan {{ $bulan ? $bulan_ref[$bulan] : '-' }} <small class="text-danger">*</small></div>
                <div>
                    <input type="number" id="realisasi" name="realisasi" class="form-control" value="{{ $realisasi_keluaran->{'keluaran_'.$bulan} }}">
                </div>
            </div>
            <div class="my-2">
                <div class="text-gray-400 text-small">Bukti Dukung <small class="text-danger">*</small></div>
                <div id="input-lampiran">
                    @if (data_get($lampiran_keluaran, "lampiran_{$bulan}"))
                    <input type="hidden" class="form-control" name="lampiran" value="{{ $lampiran_keluaran->lampiran_id }}">
                    <div class="mt-2">
                        @if (data_get($lampiran_keluaran, "lampiran_{$bulan}.tipe") == 1)
                        <div class="d-flex flex-row justify-content-between border border-success rounded px-3 py-2 text-small mb-2" style="line-height: 1">
                            <div class="text-success">{{ data_get($lampiran_keluaran, "lampiran_{$bulan}.filename") }}</div>
                            <div>
                                <a href="" data-id="{{ $lampiran_keluaran->lampiran_id }}" to="delete-attachment" bulan="{{ $bulan }}" class="text-gray-400 text-decoration-none btn-delete-attachment">
                        ...
                        @elseif (data_get($lampiran_keluaran, "lampiran_{$bulan}.tipe") == 2)
                        <textarea id="link" name="link" class="form-control" readonly>{{ data_get($lampiran_keluaran, "lampiran_{$bulan}.filename") }}</textarea>
                        <div class="mt-2">
                            <button data-id="{{ $lampiran_keluaran->lampiran_id }}" to="delete-link" bulan="{{ $bulan }}" class="btn-delete-link btn btn-sm btn-secondary">
                                Hapus
                            </button>
                        </div>
                        <script type="text/javascript">
