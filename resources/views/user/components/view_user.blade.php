...
<?php } else {
    foreach ($records as $index => $item) { ?>
        <tr style="line-height: 1;">
            <td class="text-center">{{ $item->warga_no_rumah }}</td>
            <td>
                <a href="/user/detail?id={{ $item->warga_id }}">{{ $item->warga_nama }}</a>
            </td>
            <td class="text-center"><div class="px-2 py-1 text-white rounded" style="background-color: {{ $status[$item->warga_status]['color'] }}; font-size: 0.65rem">{{ $status[$item->warga_status]['nama'] }}</div></td>
        </tr>
<?php } ?>
    <tr>
